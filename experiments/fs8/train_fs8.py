#!/usr/bin/env python
"""Training Script for fs8 Growth Rate using ALP Framework"""
...
from alp.data.datasets import load_fs8_data, preprocess_fs8_data
...
def train_fs8_model(z_train, y_train, z_test, y_test, learning_rate=0.0001,
                    batch_size=16, dropout=0.1, architecture=None, epochs=1000):
    """Train fs8 dual-output regression model using ALP framework."""
    if architecture is None:
        architecture = [200, 200, 200, 200]
    
    logger.info("Creating ALP dual-output MLP for fs8 data...")
    model = MLP(
        n_inputs=1,
        deep=architecture,
        dropout=dropout,
...
def plot_training_history(history, outdir):
    ...
    plt.title("ALP Training History - fs8 Growth Rate", fontsize=16, fontweight="bold")

    os.makedirs(outdir, exist_ok=True)
    plt.savefig(os.path.join(outdir, "training_loss.png"), dpi=150, bbox_inches="tight")
    plt.close()

    logger.info("Training history plot saved to experiments/fs8/outputs/training_loss.png")


def plot_reconstruction_comparison(results, z_test_range, z_data, fs8_data, error_data, outdir):
    """Plot fs8 reconstruction comparison."""
    ...
    plt.errorbar(
        z_data.flatten(), fs8_data.flatten(), error_data.flatten(),
        fmt="g.", markersize=2, alpha=0.4, capsize=2, capthick=1, ecolor="forestgreen",
        label="fs8 Observations", zorder=1,
    )
    ...
    plt.ylabel("f(z)σ8(z)", fontsize=16, fontweight="bold")
    plt.xticks(fontsize=14)
    plt.yticks(fontsize=14)
    plt.xlim(z_train_min - 0.05, z_train_max + 0.05)

    y_min = np.min(results_plot_mean - results_plot_unc) - 0.05
    y_max = np.max(results_plot_mean + results_plot_unc) + 0.05
    plt.ylim(y_min, y_max)

    plt.grid(True, alpha=0.3, linestyle="--")
    legend = plt.legend(loc="best", fontsize=11, framealpha=0.9, fancybox=True, shadow=True)
    legend.get_frame().set_facecolor("white")

    plt.title("fs8: Observations vs ALP Reconstruction (Training Range Only)",
    ...
    logger.info("Reconstruction comparison plot saved to experiments/fs8/outputs/reconstruction_comparison.png")


def plot_summary(history, results, z_test_range, z_data, fs8_data, outdir):
    ...
    ax2.set_ylabel("f(z)σ8(z)", fontsize=12)
    ...
    logger.info("Summary plots saved to experiments/fs8/outputs/summary_plots.png")


def main():
    """Main training function for fs8 data."""
    logger.info("Starting fs8 growth rate training with ALP framework")
    ...
    logger.info("Loading fs8 data...")
    z_data, fs8_data, error_data = load_fs8_data("data/fs8Diagram.txt")
    z_train, z_test, y_train, y_test, scaler = preprocess_fs8_data(z_data, fs8_data, error_data)
    ...
    logger.info(f"fs8 range: {np.min(fs8_data):.4f} - {np.max(fs8_data):.4f}")
    logger.info(f"error range: {np.min(error_data):.4f} - {np.max(error_data):.4f}")
    logger.info(f"Training fs8 range: {np.min(y_train[:, 0]):.4f} - {np.max(y_train[:, 0]):.4f}")

    logger.info("Training model with default hyperparameters...")
    model, history = train_fs8_model(
    ...
    os.makedirs("models", exist_ok=True)
    model.save("models/alp_fs8_model.h5")
    logger.info("Model saved to models/alp_fs8_model.h5")

    plot_training_history(history, outdir)
    plot_reconstruction_comparison(results, z_test_range, z_data, fs8_data, error_data, outdir)
    plot_summary(history, results, z_test_range, z_data, fs8_data, outdir)

    logger.info("fs8 training completed successfully!")
    logger.info("Model saved: models/alp_fs8_model.h5")
    logger.info("Results saved to experiments/fs8/outputs/")
