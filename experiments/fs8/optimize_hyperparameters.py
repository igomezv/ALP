#!/usr/bin/env python
"""Hyperparameter Optimization for fs8 Growth Rate using Optuna NSGA-II"""
...
def optimize_hyperparameters(z_train, y_train, z_test, y_test, n_trials=50):
    ...
    study = optuna.create_study(
        ...
        study_name="fs8_hyperparameters",
    )
...
def main():
    """Main hyperparameter optimization function for fs8."""
    logger.info("Starting fs8 growth rate hyperparameter optimization")
    ...
    os.makedirs("experiments/fs8/outputs", exist_ok=True)
    import json
    
    params_to_save = best_params.copy()
    params_to_save["deep"] = params_to_save["deep"]
    
    with open("experiments/fs8/outputs/best_hyperparameters.json", "w") as f:
        json.dump(params_to_save, f, indent=2)
    
    logger.info(f"Best hyperparameters saved to experiments/fs8/outputs/best_hyperparameters.json")
    logger.info("fs8 hyperparameter optimization completed successfully!")
