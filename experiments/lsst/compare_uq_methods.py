...
from alp.networks import UncertaintyQuantifier, ConformalizedQuantileRegression, create_cqr_for_sigma
...
def compare_uncertainty_methods(
    ...
    info_results_1sigma = comparison.information_criteria_comparison(
        y_test[:, 0], cqr_results_1sigma['point_prediction'],
        create_lcdm_model(), calculate_distance_modulus_range(0.01, 1.4, len(X_test), create_lcdm_model())[1],
    ...
    create_uq_comparison_plots(
        mc_results, cqr_results_1sigma, cqr_results_2sigma,
        X_test, y_test, scaler, output_dir
    )
...
def create_uq_comparison_plots(
    ...
    z_theory, mu_theory = calculate_distance_modulus_range(
        0.01, 1.4, len(X_test), cosmology
    )
    
    fig, axes = plt.subplots(2, 3, figsize=(16, 12))
    
    # Plot 1: Coverage comparison (1σ)
    ax1.bar(['MC Dropout', 'CQR 1σ'], 
    ...
    bars5 = axes[2, 0].bar(methods, precision_metrics, color=['red', 'orange', 'purple'], alpha=0.7)
    ...
    ax6.fill_between(
        z_plot, mc_plot - mc_results['interval_width'][sort_idx[:min(200, len(X_sorted))],
        mc_plot + mc_results['interval_width'][sort_idx[:min(200, len(X_sorted))],
        alpha=0.2, color='red', label='MC Dropout ±1σ'
    )
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'uq_methods_comparison.png'), dpi=300, bbox_inches='tight')
    plt.close()
...
def run_comprehensive_comparison():
    ...
    logger.info(f"   1σ Coverage - MC Dropout: {results['method_comparison']['mc_dropout']['coverage_683']:.1f}%")
    logger.info(f"   1σ Coverage - CQR 1σ: {results['method_comparison']['cqr_1sigma']['coverage_683']:.1f}%")
    logger.info(f"   Expected: 68.3%")
    
    logger.info(f"\n📊 COVERAGE PERFORMANCE:")
    logger.info(f"   2σ Coverage - CQR 2σ: {results['method_comparison']['cqr_2sigma']['coverage_955']:.1f}%")
    ...
    for level in ['1sigma', '2sigma']:
        if f'{level}_comparison' in results['information_criteria']:
            ...
            logger.info(f"   {level.upper()} - AIC: ANN={comp['aic_ann']:.0f}, Theory={comp['aic_theory']:.0f}")
            logger.info(f"   {level.upper()} - ΔAIC: {comp['delta_aic']:+.1f} (ANN {'preferred' if comp['delta_aic'] < 0 else 'worse'})")
