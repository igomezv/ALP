...
def main(heteroscedastic=False):
    ...
    # -------- 1) Load data --------
    data = pd.read_csv(data_file, names=["z", "hz", "err"], sep=r"\s+", engine="python")
    ...
    # Ensure min/max of input range are in training set
    ...
    # Add errors for newly added samples (indices refer to X_all positions)
    if added_indices:
    ...
    setup_tensorflow_for_training(seed=123, force_cpu=True)

    net = MLP(
        n_inputs=1, deep=[64, 64, 64], actfn="relu", dropout=0.1, mcdropout=True, n_outputs=2
    )
    model = net.model_tf()
    ...
    callbacks = [
        tf.keras.callbacks.ReduceLROnPlateau(
            monitor="val_loss", patience=25, factor=0.5, min_lr=1e-5, verbose=1
        ),
