#!/usr/bin/env python
"""Training Script for DES SNIa using ALP Framework"""
...
from alp.data.datasets import load_des_data, preprocess_des_data
...
def train_des_model(z_train, y_train, z_test, y_test, learning_rate=0.0001,
                    batch_size=16, dropout=0.1, architecture=None, epochs=1000):
    """Train DES dual-output regression model using ALP framework."""
    if architecture is None:
        architecture = [200, 200, 200, 200]
    
    logger.info("Creating ALP dual-output MLP for DES data...")
    model = MLP(
        n_inputs=1,
        deep=architecture,
        dropout=dropout,
...
def plot_training_history(history, outdir):
    ...
    plt.title("ALP Training History - DES SNIa", fontsize=16, fontweight="bold")

    os.makedirs(outdir, exist_ok=True)
    plt.savefig(os.path.join(outdir, "training_loss.png"), dpi=150, bbox_inches="tight")
    plt.close()

    logger.info("Training history plot saved to experiments/des5/outputs/training_loss.png")


def plot_reconstruction_comparison(results, z_test_range, z_data, mu_data, muerr_data, outdir):
    ...
    plt.errorbar(
        z_data.flatten(), mu_data.flatten(), muerr_data.flatten(),
        fmt="g.", markersize=2, alpha=0.4, capsize=2, capthick=1, ecolor="forestgreen",
        label="DES Observations", zorder=1,
    )
    ...
    plt.title("DES 5YR: Observations vs Theory vs ALP Reconstruction (Training Range Only)",
    ...
    logger.info("Reconstruction comparison plot saved to experiments/des5/outputs/reconstruction_comparison.png")
...
def plot_summary(history, results, z_test_range, z_data, mu_data, outdir):
    ...
    logger.info("Summary plots saved to experiments/des5/outputs/summary_plots.png")


def main():
    """Main training function for DES data."""
    logger.info("Starting DES SNIa training with ALP framework")
    ...
    logger.info("Loading DES data...")
    z_data, mu_data, muerr_data = load_des_data("data/DES-SN5YR_HD.csv")
    z_train, z_test, y_train, y_test, scaler = preprocess_des_data(z_data, mu_data, muerr_data)
    ...
    logger.info(f"muerr range: {np.min(muerr_data):.4f} - {np.max(muerr_data):.4f}")
    ...
    os.makedirs("models", exist_ok=True)
    model.save("models/alp_des_model.h5")
    logger.info("Model saved to models/alp_des_model.h5")

    plot_training_history(history, outdir)
    plot_reconstruction_comparison(results, z_test_range, z_data, mu_data, muerr_data, outdir)
    plot_summary(history, results, z_test_range, z_data, mu_data, outdir)

    logger.info("DES training completed successfully!")
    logger.info("Model saved: models/alp_des_model.h5")
    logger.info("Results saved to experiments/des5/outputs/")
