#!/usr/bin/env python
"""Hyperparameter Optimization for DES SNIa using Optuna NSGA-II"""
...
def optimize_hyperparameters(z_train, y_train, z_test, y_test, n_trials=50):
    ...
    study = optuna.create_study(
        ...
        study_name="des5_hyperparameters",
    )
...
def main():
    """Main hyperparameter optimization function for DES."""
    logger.info("Starting DES SNIa hyperparameter optimization")
    ...
    os.makedirs("experiments/des5/outputs", exist_ok=True)
    import json
    
    params_to_save = best_params.copy()
    params_to_save["deep"] = params_to_save["deep"]
    
    with open("experiments/des5/outputs/best_hyperparameters.json", "w") as f:
        json.dump(params_to_save, f, indent=2)
    
    logger.info(f"Best hyperparameters saved to experiments/des5/outputs/best_hyperparameters.json")
    logger.info("DES hyperparameter optimization completed successfully!")
