...
class Cosmology:
    ...
    def dist_integrand_a(self, a: float) -> float:
        ...
        """
        return 1./np.sqrt(self.rhsquared_a_owacdm(a))/a**2
...
def create_wacdm_model(H0: float = 70.0, Om: float = 0.27, 
                     w0: float = -1.0, wa: float = 0.0) -> Cosmology:
...
def calculate_distance_modulus_range(z_min: float = 0.01, z_max: float = 2.4, 
                               n_points: int = 100, 
                               cosmology: Cosmology = None) -> Tuple[np.ndarray, np.ndarray]:
